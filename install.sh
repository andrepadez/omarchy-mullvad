...
check_dependencies() {
  ...
  for cmd in mullvad walker jq; do
...
create_share_symlink() {
  ...
  local symlink_path="${share_bin_dir}/omarchy-mullvad"
...
main() {
  ...
  echo -e "${BLUE}╔════════════════════════════════════════╗${NC}"
  if [ "$DRY_RUN" = true ]; then
    echo -e "${BLUE}║  ${YELLOW}Omarchy Mullvad Installer (DRY-RUN)${BLUE}  ║${NC}"
  else
    echo -e "${BLUE}║  ${YELLOW}Omarchy Mullvad Installer${BLUE}       ║${NC}"
  fi
  echo -e "${BLUE}╚════════════════════════════════════════╝${NC}"
  echo ""
  ...
  echo -e "${GREEN}╚════════════════════════════════════════╝${NC}"
  echo ""
  print_success "You can now run: ${YELLOW}omarchy-mullvad.sh${NC}"
  print_info "Script installed to: $INSTALL_DIR"
  echo ""
  print_info "Usage:"
  echo "  omarchy-mullvad.sh          # Launch interactive menu"
  echo "  omarchy-mullvad.sh status   # Show JSON status (for Waybar)"
  echo "  omarchy-mullvad.sh switch   # Toggle connection"
  echo ""
  echo ""
